...
- **验收**:代码层 ✅(四 provider 配置解析 + wizard 端到端 mock 已过);真实 GLM/OpenAI 问答 + OLE 登录实测待用户填 key/凭证后验证(2026-07-02)
...
**Phase 1 — BYO-key:✅ 已完成(2026-07-02)**,见 §6(`aedf0e2` + `ec04ded`)。
真实 API 问答 + OLE 登录实测待用户填 key/凭证后验证。
下一阶段 **Phase 2 — 成熟度补课**(流式输出 / 上下文管理 / 工具调用健壮性 / 错误重试),等用户发令后启动。
