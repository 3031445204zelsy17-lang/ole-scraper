...
- [x] PDF/讲义抽文本(PyMuPDF 逐页 + 字符滑窗分块 350 字 / 60 overlap)(753c673 / e49a2f3)
- [x] 嵌入:本地 bge-small-zh-v1.5 + sentence-transformers(cpu / normalize)(§8 ⑥,arch-check 实查)
- [x] 轻量向量索引:numpy 手搓(np.save + chunks.json;arch-check 实测几千向量亚毫秒)(e49a2f3)
- [x] 接入 `agent_loop` 作为新工具 `retrieve_course_content`(b8deb80)
- [x] 引用来源:回答标注 PDF + 页码(端到端实测 Tutorial 6 → 引用 pdf p4-5)
- **验收**:✅ 实测「Tutorial 6 介绍了哪些离散分布」→ agent 调 retrieve 两次 + 基于片段答 Geometric/Negative Binomial/Poisson + 引用来源(2026-07-04)

### Phase 3.5 — 前端打磨(React SPA)
**目标**:前端从单 HTML 升级 React SPA:仿 Claude.ai 视觉 + markdown + 多会话 + RAG 来源可点击。
- [x] P3.5-1 Vite+React+TS 脚手架 + FastAPI `app.frontend()` 接入(48abeeb)
- [x] P3.5-2 仿 Claude 聊天 UI + markdown 渲染(634ef97)
- [x] P3.5-3 多会话侧边栏(Zustand + 持久化 + restore)(161365d)
- [x] P3.5-4 RAG 来源可点击(跳原文 PDF)(ca99fd8)
- [x] P3.5-5 生产托管(`app.frontend` dist)+ `init.sh` 集成前端构建
- **验收**:Playwright 实测流式/markdown/多会话/来源可点击;`npm run build` 产 dist(无 TS 错);`init.sh` 一键(装+build+起)。栈:React 19.2 + Vite 8 + TS 6 + Tailwind v4 + Zustand + react-markdown(arch-check 实查,2026-07-04)。

### Phase 3.6 — 官网公开信息 RAG(引用式)
**目标**:覆盖 OLE + 课件之外的第三类——HKMU 官网(www.hkmu.edu.hk)公开信息(招生/专业/新闻/校历等)。
**合规前提**:官网是 WordPress+Elementor(SSR);robots.txt 带 Cloudflare AI 信号(`search=yes, ai-train=no, use=reference`),RAG 属其定义的 `ai-input`(未明确授权)。决议:**保守推进、引用式**(片段+来源外链、不全文复现、仅本地个人自用、不训练/不对外服务)。详见 §8 ⑦ 与 README「官网公开信息合规说明」。
- [x] P3.6-0 httpx + trafilatura + bs4/lxml + 目录/.gitignore(eac550c)
- [x] P3.6-1 public_crawler:sitemap 发现(10 精选子站)→ httpx 抓(诚实 UA、~2 req/s)→ bs4 剥 cookie consent → trafilatura 抽正文 → 缓存;英文过滤(/sc/ /tc/ + CJK slug)+ 每子站按 lastmod 取最新 400(种子必留;e0ea797)
- [x] P3.6-2 rag_index 抽 `_search_index`(共享检索,PDF 路径行为不变;18c1bf1)
- [x] P3.6-3 public_rag build/retrieve + `retrieve_public_info` 工具 + agent 引导(b2ecd57)
- [x] P3.6-4 README 合规姿态 + 本节
- [x] P3.6-5 嵌入模型:官网索引用 **bge-small-en-v1.5**(英文专精),PDF 保持 bge-small-zh;`meta.json` 记录每索引模型,检索按此加载(51b5fa0)
- [x] P3.6-6 鲁棒性修复(实测「DSAI Y2 课程」暴露):防课程编造 + `retrieve_*` 硬上限 3 次/轮收敛 + LLM 查询自翻英文官方名(d95c00b)
- 🔬 BM25+向量混合检索已试,**在 HKMU ~30 个雷同专业页语料上净负向**(常见词 programmes/subsidized/year flood,好结果被顶下去),已回退
- **验收**:3166 页 → 48818 块(en,384 维);常见查询(招生/SSSDP/图书馆/奖学金/新闻/专业概览)检索+回答正常,来源可点击;**已知局限**:小模型纯向量在 ~30 个结构雷同的专业页间区分度有限,「某专业某学期具体课表」类查询 agent **诚实降级**(给官网 URL + 建议 OLE 查,不编造)。彻底解决需更大模型或商用嵌入 API(见 §8 ⑧,后续)。
...
| ⑥ | RAG 完整栈(Phase 3) | `bge-small-zh-v1.5` + `sentence-transformers` + numpy 手搓向量 + PyMuPDF/pymupdf4llm(AGPL 组件,本地自用零义务) | 2026-07-04 arch-check 实查 |
| ⑦ | 官网 RAG 合规姿态(Phase 3.6) | 精选公开子站(主站+招生+7 学院+图书馆+新闻,排除个人门户)+ 仅英文 + **引用式**(片段+外链、不全文复现、仅本地个人自用);认知 robots `use=reference` 信号,不做训练/对外服务 | 2026-07-05 |
| ⑧ | 官网嵌入模型 + 检索天花板(Phase 3.6-5/6) | 官网索引用 bge-small-en-v1.5(英文),PDF 保持 bge-small-zh(`meta.json` 记录每索引模型);BM25+向量混合经实测在本语料净负向(常见词 flood)已回退;**小模型纯向量是当前天花板**——精确「某专业某学期课表」类有限,agent 诚实降级(给 URL)。彻底提升需 bge-base/large-en 或商用嵌入 API(后续可选) | 2026-07-05 |
...
**Phase 3.6 — 官网公开信息 RAG:✅ 已完成(2026-07-05)**,见 §6(`eac550c` → `b2ecd57` + 本提交)。
新增 HKMU 官网精选公开页面的**引用式** RAG(sitemap → httpx → trafilatura → bge,复用 Phase 3 的 `_search_index`),来源为可点击外链;合规边界见 §8 ⑦ + README。
下一阶段 **Phase 4 — 分发**(面向他人的 init.sh / requirements + Mac/Win 安装文档),等用户发令后启动。
