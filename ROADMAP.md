...
- [x] 补 `.gitignore`:`sessions/` `downloads/` `logs/` `ole-data/current/` + `ole-downloads/` `ole-session/`(bd9f76c)
- [x] 去硬编码:`agent_loop.py:41` + `tools.py:138` 的 `/Users/<个人>/Desktop` → 默认 `downloads/<course>/`(82f7f83)
- [x] 移除个人数据:`ole-data/current/*.json` + 课件 PDF → 换成 `example/` 示例(c00cb8b,本地保留)
- [x] 排查硬编码课程码:`app/` 仅 `main.py` 帮助文本的 COMP2090(示例,非泄漏,保留)
- [x] 归档退役文件:`DEV_PLAN.md` / `feature_list.json` → `archive/`(7cc0f35)
- [x] 重写 `CLAUDE.md` 架构段(脱节)+ `README`(定位 + 隐私 + 免责)
- [x] 补项(原 §4 已列但 §6 漏排):`.env.example` 真实凭证脱敏(bd9f76c);删 3 处死代码 `retriever.py`/`schemas.py`/`agent.py` → 建 `config.py`(82f7f83)
...
- [ ] 多 provider 抽象:OpenAI 兼容接口,支持 DeepSeek / GLM / OpenAI / Ollama(provider + base_url + key + model 可配)
- [ ] 去掉 `agent_loop.py` 里 hardcode 的 `DEEPSEEK_URL` / `"deepseek-chat"` ← **agent_loop 主线**
- [ ] 首启 wizard:首次运行收 `LLM_API_KEY` + HKMU 凭证 → 写本地 `.env` / session
- [ ] `.env.example` 更新为多 provider 模板
- **验收**:用 GLM/OpenAI key 能跑通一次问答;首启引导流程能完成登录。

### Phase 2 — 成熟度补课(用户体验)
**目标**:用起来像主流 agent,不像脚本。(依赖:Phase 1)
- [ ] 流式输出:backend 改 stream → frontend 逐字渲染
- [ ] 上下文管理:摘要替代粗暴截断;历史持久化到本地(刷新不丢)
- [ ] 工具调用健壮性:参数 schema 校验 + JSON 解析失败重试;**删掉** `file_type` 手写补丁 ← **agent_loop 主线**
- [ ] 错误处理:网络错误退避重试,不再单次失败即放弃
- **验收**:长会话(>6 轮)不丢早期上下文;工具参数错误有恢复而非静默;回答逐字出现。
...
**Phase 0 — 清场:✅ 已完成(2026-07-02)**,见 §6 勾选项(commit bd9f76c → 7cc0f35 + 本提交)。
下一阶段 **Phase 1 — BYO-key**(多 provider 抽象 + 去掉 hardcode DeepSeek + 首启 wizard),等用户发令后启动。
