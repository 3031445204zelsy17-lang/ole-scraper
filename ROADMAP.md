...
- **验收**:小规模端到端 6 页 → 69 块 → 检索「undergraduate programmes SSSDP」命中 SSSDP / 商学院项目页 + 可点击外链;handler 返回正确。完整 crawl(~3000 页,可续抓)由用户本机运行。
...
| ⑦ | 官网 RAG 合规姿态(Phase 3.6) | 精选公开子站(主站+招生+7 学院+图书馆+新闻,排除个人门户)+ 仅英文 + 复用 bge-small-zh + **引用式**(片段+外链、不全文复现、仅本地个人自用);认知 robots `use=reference` 信号,不做训练/对外服务 | 2026-07-05 |
