...
4. RAG 检索工具（本地索引，需先建索引）：
   ...
   - retrieve_public_info: 从 HKMU 官网公开页面回答「招生/专业/学费/奖学金/新闻/校历/图书馆服务」(需 `python -m app.public_rag crawl && build`)
...
- 用户问 HKMU 官网公开信息(招生政策/专业介绍/学费/奖学金/新闻/校历/图书馆服务等),OLE 缓存与课件 RAG 都覆盖不到 → retrieve_public_info(传 query 关键词)
